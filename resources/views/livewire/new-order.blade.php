<div x-data="{
    ...
    products: {{ json_encode($extraProducts->flatten()->map(fn($p) => [
        'id'       => $p->id,
        'name'     => $p->name,
        'category' => $p->category,
    ...
    prices: {{ json_encode(['p199' => $prices['basic'], 'p269' => $prices['premium'], 'p349' => $prices['deluxe'], 'addon' => $prices['addon']]) }},
    ...
    <div wire:ignore>
    {{-- ══════════════════════════════════════════════════════
         NEW ORDER — TICKET BOARD LAYOUT
         Top: Package selector (full width horizontal cards)
         Bottom-left: Extras grid  |  Bottom-right: Bill/charge
    ══════════════════════════════════════════════════════ --}}
    <div class="ord-layout">

        {{-- ═══ ROW 1: Package selector cards ═══ --}}
        <div class="ord-pkg-row">
            ...
            <div class="ord-pkg-cards">
                ...
                <div class="ord-pkg-card" :class="pkgs.{{ $pkg['key'] }} > 0 ? 'ord-pkg-on' : ''"
                     :style="pkgs.{{ $pkg['key'] }} > 0 ? 'border-color:{{ $pkg['color'] }};box-shadow:0 0 0 3px {{ $pkg['color'] }}30,var(--shadow-md);background:{{ $pkg['glow'] }};' : ''">
                    <div class="ord-pkg-top">
                        <span class="ord-pkg-badge" style="background:{{ $pkg['color'] }}22;color:{{ $pkg['color'] }};border:1.5px solid {{ $pkg['color'] }}55;">{{ $pkg['label'] }}</span>
                        <span class="ord-pkg-price">₱{{ $pkg['price'] }}<span class="ord-pkg-pax">/pax</span></span>
                    </div>
                    <div class="ord-pkg-ctrl">
                        <button class="ord-qbtn ord-qbtn-minus" type="button"
                                title="Remove one person"
                                @click="changePkg('{{ $pkg['key'] }}', -1)">−</button>
                        <span class="ord-pkg-count-box">
                            <span class="ord-pkg-count-num" x-text="pkgs.{{ $pkg['key'] }} || 0"></span>
                            <span class="ord-pkg-count-lbl">pax</span>
                        </span>
                        <button class="ord-qbtn ord-qbtn-plus" type="button"
                                title="Add one person"
                                style="border-color:{{ $pkg['color'] }}66;color:{{ $pkg['color'] }};background:{{ $pkg['color'] }}15;"
                                @click="changePkg('{{ $pkg['key'] }}', 1)">+</button>
                    </div>
                </div>
                @endforeach
        ...
        <div class="ord-options-row">
            ...
            <div class="ord-options-inner">
                ...
                <div class="ord-options-section">
                    <div class="ord-options-section-title">🍶 Add-ons <span style="color:var(--text-3);font-size:13px;font-weight:500;">₱{{ $prices['addon'] }} per person</span></div>
                    <div class="ord-addons-row">
                        @foreach([['key'=>'icedTea','emoji'=>'🧊','label'=>'Iced Tea'],['key'=>'cheese','emoji'=>'🧀','label'=>'Cheese']] as $addon)
                        <div class="ord-addon-card" :class="addons.{{ $addon['key'] }} > 0 ? 'ord-addon-on' : ''">
                            <div class="ord-addon-top">
                                <span class="ord-addon-emoji">{{ $addon['emoji'] }}</span>
                                <span class="ord-addon-name">{{ $addon['label'] }}</span>
                                <span class="ord-addon-price">₱{{ $prices['addon'] }}</span>
                            </div>
                            <div class="ord-addon-ctrl">
                                <button class="ord-qbtn ord-qbtn-lg ord-qbtn-minus" type="button"
                                        @click="addons.{{ $addon['key'] }} = Math.max(0,(addons.{{ $addon['key'] }}||0)-1)">−</button>
                                <span class="ord-addon-count" x-text="addons.{{ $addon['key'] }} || 0"></span>
                                <button class="ord-qbtn ord-qbtn-lg ord-qbtn-plus" type="button"
                                        @click="addons.{{ $addon['key'] }} = (addons.{{ $addon['key'] }}||0)+1">+</button>
                            </div>
                        </div>
                        @endforeach
                    </div>
        ...
        {{-- ═══ ROW 3: Extras panel (left column) ═══ --}}
        ...
        {{-- ═══ BILL PANEL — sticky right column, spans all rows ═══ --}}
        <div class="ord-bill-panel">
            ...
            {{-- Line items: always rendered, items hidden via x-show --}}
