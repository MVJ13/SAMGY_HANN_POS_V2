<div>
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
        ...
        <span style="color:#999;font-size:0.85em;">Extras are managed via the <strong
                style="color:#ff6b6b;">Inventory</strong> tab</span>
    </div>
    ...
    <div
        style="background:#3a1a1a;border:1px solid #f44336;border-radius:8px;padding:14px 18px;margin-bottom:18px;color:#f44336;font-weight:600;">
        ...
        <div class="form-group">
            ...
            <div class="package-selector">
                <div class="package-card">
                    <h3>Basic — ₱199</h3>
                    <input type="number" x-model.number="pkgs.p199" @input="setPkg('p199', $event.target.value)" min="0"
...
unset($__errorArgs, $__bag); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                ...
                <div class="package-card">
                    <h3>Premium — ₱269</h3>
                    <input type="number" x-model.number="pkgs.p269" @input="setPkg('p269', $event.target.value)" min="0"
                        placeholder="# of people">
                    <p style="margin-top:10px;color:#999;">per person</p>
                </div>
                <div class="package-card">
                    <h3>Deluxe — ₱349</h3>
                    <input type="number" x-model.number="pkgs.p349" @input="setPkg('p349', $event.target.value)" min="0"
                        placeholder="# of people">
                    <p style="margin-top:10px;color:#999;">per person</p>
                </div>
            </div>
        ...
        <div class="form-group">
            ...
            <div class="two-col">
                <div class="addon-card">
                    <label>🧊 Unlimited Iced Tea</label>
                    <input type="number" x-model.number="addons.icedTea"
                        @input="setAddon('icedTea', $event.target.value)" min="0" placeholder="# of people">
                </div>

                <div class="addon-card">
                    <label>🧀 Unlimited Cheese</label>
                    <input type="number" x-model.number="addons.cheese" @input="setAddon('cheese', $event.target.value)"
        ...
        <div class="form-group">
            ...
            <div class="extras-category">
                ...
                <div class="extras-grid">
                    ...
                    <?php $outOfStock = $product->stock <= 0; ?> <div class="extra-item-card"
                        ...
                        <div
                            style="font-size:0.75em;margin-bottom:4px;color:<?php echo e($outOfStock ? '#f44336' : ($product->is_low_stock ? '#ff9800' : '#4caf50')); ?>;">
                        ...
                        <div class="extra-qty-control">
                            <button type="button" class="qty-btn" @click="decrement(<?php echo e($product->id); ?>)">−</button>
                            <input type="number" class="qty-input" :value="qty(<?php echo e($product->id); ?>)"
                                @change="set(<?php echo e($product->id); ?>, $event.target.value)" min="0"
                                max="<?php echo e((int) $product->stock); ?>">
    ...
    <div class="two-col">
        ...
        <div class="form-group">
            ...
            <input type="number" x-model.number="discount" @input="setDiscount($event.target.value)" min="0" max="100"
    ...
    <template x-if="hasAnything">
        <div class="order-summary">
            ...
            <template x-if="pkgs.p199 > 0">
                <p>• Basic (₱199): <span x-text="pkgs.p199"></span> <span
                        x-text="pkgs.p199 === 1 ? 'person' : 'people'"></span> = ₱<span
                        x-text="fmt(pkgs.p199 * 199)"></span></p>
            </template>
            <template x-if="pkgs.p269 > 0">
                <p>• Premium (₱269): <span x-text="pkgs.p269"></span> <span
                        x-text="pkgs.p269 === 1 ? 'person' : 'people'"></span> = ₱<span
                        x-text="fmt(pkgs.p269 * 269)"></span></p>
            </template>
            <template x-if="pkgs.p349 > 0">
                <p>• Deluxe (₱349): <span x-text="pkgs.p349"></span> <span
                        x-text="pkgs.p349 === 1 ? 'person' : 'people'"></span> = ₱<span
                        x-text="fmt(pkgs.p349 * 349)"></span></p>
            </template>

            
            <template x-if="addons.icedTea > 0">
                <p>• Unlimited Iced Tea: <span x-text="addons.icedTea"></span> <span
                        x-text="addons.icedTea === 1 ? 'person' : 'people'"></span> × ₱25 = ₱<span
                        x-text="fmt(addons.icedTea * 25)"></span></p>
            </template>
            <template x-if="addons.cheese > 0">
                <p>• Unlimited Cheese: <span x-text="addons.cheese"></span> <span
                        x-text="addons.cheese === 1 ? 'person' : 'people'"></span> × ₱25 = ₱<span
                        x-text="fmt(addons.cheese * 25)"></span></p>
            </template>

            
            <template x-for="item in activeExtras" :key="item.id">
                <p>• <span x-text="item.name"></span>: <span x-text="item.qty"></span> × ₱<span
                        x-text="fmt(item.price)"></span> = ₱<span x-text="fmt(item.qty * item.price)"></span></p>
            </template>
            ...
            <div style="border-top:2px solid #424242;margin:15px 0;padding-top:15px;">
                ...
                <template x-if="discount > 0">
                    <p><strong>Discount (<span x-text="discount"></span>%):</strong>
                        <span style="color:#4caf50;">-₱<span x-text="fmt(discountAmount)"></span></span>
                    </p>
                </template>
                <p><strong>Total:</strong>
                    <span style="color:#ff6b6b;font-size:1.5em;font-weight:700;">₱<span
                            x-text="fmt(total)"></span></span>
                </p>
            </div>
        </div>
    </template>
    ...
    <button class="btn btn-primary" wire:loading.attr="disabled" wire:loading.class="btn-loading"
        @click="submitOrder()">
...
</script><?php /**PATH C:\Users\Mark\samgyeopsal-pos\resources\views/livewire/new-order.blade.php ENDPATH**/ ?>